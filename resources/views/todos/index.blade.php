<x-app-layout>
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('error'))
                <div class="mb-4 p-4 bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-600 text-red-800 dark:text-red-200 rounded">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                @if($canCreateTodos)
                <div>
                    <a href="{{ route('todos.create') }}" class="inline-flex items-center px-4 py-2 bg-[#8B0116] dark:bg-[#C41E3A] border border-transparent rounded-md font-semibold text-white hover:bg-[#A50019] dark:hover:bg-[#D63A4D] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#8B0116] dark:focus:ring-[#FF6B81]">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-2">
                                            @if($todo->status === 'assigned')
                                                <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-xs">In Bearbeitung</span>
                                            @elseif($todo->status === 'completed')
                                                <span class="px-2 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded-full text-xs">Wartet auf Verifizierung</span>
                                            @elseif($todo->status === 'verified')
                                                <span class="px-2 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-full text-xs">Verifiziert</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-2 text-gray-700 dark:text-gray-300">{{ $todo->points }}</td>
                                        <td class="px-4 py-2 text-center">
                                            <a href="{{ route('todos.show', $todo) }}" class="text-[#8B0116] dark:text-[#FF6B81] hover:underline">
                                                Details
                                            </a>
                        ...
                        @foreach($assignedTodos as $todo)
                            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('todos.show', $todo) }}" class="inline-block bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-800 dark:text-white py-1 px-3 rounded text-sm">
                                        Details
                                    </a>
                                    ...
                                    @if($todo->status === 'assigned')
                                        <form action="{{ route('todos.complete', $todo) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="inline-block bg-green-500 hover:bg-green-600 text-white py-1 px-3 rounded text-sm">
                                                Als erledigt markieren
                                            </button>
                                        </form>
                                    @endif
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($unassignedTodos as $todo)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-gray-700 dark:text-gray-300">{{ $todo->points }}</td>
                                        <td class="px-4 py-2 text-center">
                                            <a href="{{ route('todos.show', $todo) }}" class="text-[#8B0116] dark:text-[#FF6B81] hover:underline mr-2">
                                                Details
                                            </a>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <form action="{{ route('todos.assign', $todo) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="inline-block bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded text-sm">
                                            Übernehmen
                                        </button>
                                    </form>
                                </div>
            ...
            @if($canVerifyTodos && $completedTodos->where('status', 'completed')->isNotEmpty())
                <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg p-6">
                    <h2 class="text-xl font-semibold text-[#8B0116] dark:text-[#FF6B81] mb-4">Zu verifizierende Aufgaben</h2>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($completedTodos->where('status', 'completed') as $todo)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-gray-700 dark:text-gray-300">{{ $todo->completed_at->format('d.m.Y H:i') }}</td>
                                        <td class="px-4 py-2 text-gray-700 dark:text-gray-300">{{ $todo->points }}</td>
                                        <td class="px-4 py-2 text-center">
                                            <a href="{{ route('todos.show', $todo) }}" class="text-[#8B0116] dark:text-[#FF6B81] hover:underline mr-2">
                                                Details
                        ...
                        @foreach($completedTodos->where('status', 'completed') as $todo)
                            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow">
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="mt-1 text-gray-800 dark:text-gray-200">{{ $todo->completed_at->format('d.m.Y H:i') }}</div>
                                </div>
                                <div class="mb-3">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Punkte:</span>
                                    <div class="mt-1 font-semibold text-gray-800 dark:text-white">{{ $todo->points }}</div>
