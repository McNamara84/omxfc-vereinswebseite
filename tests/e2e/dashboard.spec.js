...
test.describe('Dashboard overview', () => {
    test('admin sees dashboard insights, applicants and verification card', async ({ page }) => {
        ...
        const cards = page.locator('div[aria-label="ÃƒÅ“berblick wichtiger Community-Kennzahlen"] [role="region"]');
        await expect(cards).toHaveCount(6);

        await expect(page.getByTestId('dashboard-applicants-panel')).toBeVisible();
        await expect(page.getByTestId('dashboard-applicant-row').filter({ hasText: 'Playwright AnwÃƒÂ¤rter' })).toBeVisible();
